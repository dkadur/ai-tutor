          ...
          >
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center justify-center gap-3.5">
                <Image src="/images/star.svg" alt="progress star" width={28} height={28} className="md:w-9 md:h-9" />
                <p className="text-2xl font-bold md:text-3xl">{exam.name}</p>
              </div>
              <div>
                <div className="hidden md:block">
                  <Button asChild>
                    <Link href={`/exam/${exam.exam_id}/${exam.name.toLowerCase().replaceAll(" ", "-").replaceAll("#", "")}/instruction`}>Take exam!</Link>
                  </Button>
                </div>
              </div>
            </div>
